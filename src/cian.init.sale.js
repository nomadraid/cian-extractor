...
async function init() {
    ...
    let collection = db.collection('sale_offers_ids');
    let checkCollection = db.collection('sale_offers_data');
...
function buildSearchURL(points) {
    ...
    return 'https://map.cian.ru/ajax/map/roundabout/?deal_type=sale&engine_version=2&in_polygon[0]=' + area.join(',');
}
